...
namespace mzn {
CxSerialInterface1Status::CxSerialInterface1Status():
    Command(0, 36),
...
uint16_t CxSerialInterface1Status::msg_to_data(std::vector<uint8_t> const & msg,
                                               uint16_t mf_begin) {

    if ( msg.size()  < cmd_data_size_ + mf_begin) {
        throw FatalException(
            "CxSerialInterface1Status",
            "msg_to_data",
...
uint16_t CxSerialInterface1Status::data_to_msg(std::vector<uint8_t> & msg,
...
std::ostream & CxSerialInterface1Status::os_print(std::ostream & cmd_os) const {
    cmd_os << "\n --- CX_SERIAL_INTERFACE_1_STATUS ---  \n";
